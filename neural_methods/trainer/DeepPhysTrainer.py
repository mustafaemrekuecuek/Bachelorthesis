...
class DeepPhysTrainer(BaseTrainer):
    ...
    def __init__(self, config, data_loader):
        ...
        # TDCM config
        self.use_tdcm = hasattr(config.MODEL, "TDCM") and config.MODEL.TDCM.ENABLE

        self.tdcm_config = None
        if self.use_tdcm:
            self.tdcm_config = {
                "CHANNELS": config.MODEL.TDCM.CHANNELS,
                "DROPOUT": config.MODEL.TDCM.DROPOUT,
                "KERNEL_SHORT": config.MODEL.TDCM.KERNEL_SHORT,
                "KERNEL_MID": config.MODEL.TDCM.KERNEL_MID,
                "KERNEL_LONG": config.MODEL.TDCM.KERNEL_LONG,
                "DILATION_MID": config.MODEL.TDCM.DILATION_MID,
                "DILATION_LONG": config.MODEL.TDCM.DILATION_LONG,
            }
    ...
    def train(self, data_loader):
        ...
        for epoch in range(self.max_epoch_num):
            ...
            for idx, batch in enumerate(tbar):
                ...
                if self.use_tdcm:
                    # Keep temporal dimension for TDCM
                    labels = labels.view(N, D, 1)
                    pred_ppg = self.model(data)  # [N, D, 1]
                else:
                    # Original DeepPhys behavior
        ...
        with torch.no_grad():
            ...
            for valid_idx, valid_batch in enumerate(vbar):
                ...
                if self.use_tdcm:
                    ...
                    pred_ppg_valid = self.model(data_valid)  # [N, D, 1]
                else:
        ...
        with torch.no_grad():
            for _, test_batch in enumerate(tqdm(data_loader["test"], ncols=80)):
                ...
                if self.use_tdcm:
                    ...
                    pred_ppg_test = self.model(data_test)  # [N, D, 1]
                else:
                ...
                for idx in range(batch_size):
                    ...
                    if self.use_tdcm:
                        predictions[subj_index][sort_index] = pred_ppg_test[idx].squeeze(-1)
                        labels[subj_index][sort_index] = labels_test[idx].squeeze(-1)
                    else:
